      ...
      >
        <Box w="100%" >
          ...
          >
            ...
            <TruncatedValue isLoading={ isLoading } color="#F39B4B" fontWeight={ 600 }
              fontSize="lg" lineHeight={ 6 } value={ value }/>
            { valuePostfix && (
              <chakra.span fontWeight={ 400 } color="#F39B4B" fontSize="lg" lineHeight={ 6 } whiteSpace="pre">
                { valuePostfix }
              </chakra.span>
            ) }
            { diff && Number(diff) > 0 && (
              <>
                <Text fontWeight={ 600 } ml={ 2 } mr={ 1 } fontSize="lg" lineHeight={ 6 } color="green.500">
                  +{ diffFormatted || Number(diff).toLocaleString() }
                </Text>
                <Text variant="secondary" fontSize="sm" color="#F39B4B">({ diffPeriod })</Text>
              </>
            ) }
            { period && (
              <Text variant="secondary" fontSize="xs" ml={ 1 }
                color="#F39B4B"
              >({ period })</Text>
            ) }
          </Skeleton>
